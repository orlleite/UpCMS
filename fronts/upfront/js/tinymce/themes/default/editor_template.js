...
(function(tinymce) {
	...
	tinymce.ThemeManager.requireLangPack('default');

	tinymce.create('tinymce.themes.DefaultTheme', {
		...
		init : function(ed, url) {
			...
			ed.onInit.add(function() {
				...
				if (ed.settings.content_css !== false)
					ed.dom.loadCSS(ed.baseURI.toAbsolute("themes/default/skins/" + ed.settings.skin + "/content.css"));
			});
		...
		createControl : function(n, cf) {
			var cd, c, fb = false, lb = false, ub = false;
			...
			if (c = cf.createControl(n)) {
				if( ub ) c.classPrefix += ' mceUniqueButton';
				else if( fb ) c.classPrefix += ' mceFirstButton';
				else if( lb ) c.classPrefix += ' mceLastButton';
			...
			{
				var o = { title : "advanced." + cd[0], cmd : cd[1], ui : cd[2], value : cd[3] };

				c = cf.createButton(n, o);

				if( ub ) c.classPrefix += ' mceUniqueButton';
				else if( fb ) c.classPrefix += ' mceFirstButton';
				else if( lb ) c.classPrefix += ' mceLastButton';
				
				if( n == "link" ) alert( c.classPrefix );
...
*/
		...
		getInfo : function() {
			return {
				longname : 'Default UP Theme',
				author : 'orlleite',
				authorurl : 'http://orlleite.com',
		...
		_simpleLayout : function(s, tb, o, p) {
			...
			if (s.readonly) {
				...
				DOM.add(tr, 'td', {'class' : 'mceContainerLeft'});
				n = ic = DOM.add(tr, 'td', {'class' : 'mceIframeContainer'});
				DOM.add(tr, 'td', {'class' : 'mceContainerRight'});
				return ic;
			}
		...
		_addToolbars : function(c, o) {
			...
			var tr = DOM.add(c, 'tr');
			DOM.add(tr, 'td', {'class' : 'mceToolbarLeft'});
			n = DOM.add(tr, 'td', {'class' : 'mceToolbar mceToolbarCenter ' + a});
			DOM.add(tr, 'td', {'class' : 'mceToolbarRight'});
		...
		_addStatusBar : function(tb, o) {
			...
			var tr = DOM.add(tb, 'tr');
			DOM.add(tr, 'td', {'class' : 'mceStatusbarLeft'});
			n = td = DOM.add(tr, 'td', {'class' : 'mceStatusbar'});
			DOM.add(tr, 'td', {'class' : 'mceStatusbarRight'});
...
*/
	...
	tinymce.ThemeManager.add('default', tinymce.themes.DefaultTheme);
}(tinymce));